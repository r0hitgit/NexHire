...
export default function RecruiterDashboard() {
  ...
  const glassModal = {
    ...
    borderRadius: "var(--radius-lg)",
    boxShadow: "0 16px 48px rgba(0,0,0,0.5), 0 0 0 1px rgba(108,99,255,0.1)",
  };
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <main style={{ maxWidth: "1200px", margin: "0 auto", padding: "clamp(1rem, 4vw, 2.5rem) clamp(1rem, 3vw, 2rem)" }}>
        ...
        <div className="dashboard-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr", gap: "1.5rem", alignItems: "start" }}>
          ...
          {/* Jobs Panel */}
          <div className={`panel-jobs ${showApplicants ? "hide-mobile" : ""}`} style={glassCard}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "0.5rem" }}>
                    ...
                    <div style={{ display: "flex", gap: "0.4rem", flexShrink: 0 }}>
                      <button onClick={(e) => handleOpenEdit(e, job)} style={{
                        ...
                        onMouseLeave={e => { e.currentTarget.style.background = "rgba(108,99,255,0.1)"; e.currentTarget.style.color = "#a09aff"; }}
                      >Edit</button>
                      <button onClick={(e) => handleDelete(e, job.id)} style={{
                        ...
                        onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,101,132,0.18)"; }}
                        onMouseLeave={e => { e.currentTarget.style.background = "rgba(255,101,132,0.08)"; }}
                      >Delete</button>
                    </div>
                  </div>
                </div>
          ...
          {/* Applicants Panel */}
          <div className={`panel-applicants ${!showApplicants ? "hide-mobile" : ""}`} style={glassCard}>
                ...
                <div key={app.id} style={{
                  ...
                  {app.resumeUrl && (
                    <a href={app.resumeUrl} target="_blank" rel="noopener noreferrer" style={{
                      ...
                      border: "1px solid rgba(108,99,255,0.25)", borderRadius: "20px",
                  ...
                  <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap", marginBottom: "0.75rem" }}>
                    {["APPLIED", "SHORTLISTED", "REJECTED"].map(st => (
                      <button key={st} onClick={() => handleStatus(app.id, st)} style={{
                        ...
                        border: `1px solid ${STATUS_COLORS[st]}35`,
                        background: app.status === st ? `${STATUS_COLORS[st]}18` : "rgba(255,255,255,0.03)",
                        color: app.status === st ? STATUS_COLORS[st] : "var(--text2)",
                        fontSize: "0.72rem", fontWeight: 600, transition: "all 0.2s ease",
                  ...
                  <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                    <button onClick={() => setInterviewModal(app.id)} style={{
                      ...
                      border: "1px solid rgba(67,233,123,0.25)", fontSize: "0.78rem",
                      fontWeight: 600, cursor: "pointer", backdropFilter: "blur(8px)",
                      transition: "all 0.2s ease",
                    }}
                      onMouseEnter={e => { e.currentTarget.style.background = "rgba(67,233,123,0.16)"; e.currentTarget.style.borderColor = "rgba(67,233,123,0.45)"; }}
                      onMouseLeave={e => { e.currentTarget.style.background = "rgba(67,233,123,0.08)"; e.currentTarget.style.borderColor = "rgba(67,233,123,0.25)"; }}
                    >📅 Schedule Interview</button>

                    <button onClick={() => setContactModal(app.id)} style={{
                      padding: "0.35rem 0.85rem", borderRadius: "var(--radius)",
                      background: "rgba(245,158,11,0.08)", color: "#f59e0b",
                      border: "1px solid rgba(245,158,11,0.25)", fontSize: "0.78rem",
                      fontWeight: 600, cursor: "pointer", backdropFilter: "blur(8px)",
                      transition: "all 0.2s ease",
                    }}
                  ...
                  {app.status === "INTERVIEW_SCHEDULED" && app.interviewScheduledAt && (
                    <div style={{
                      ...
                      background: "rgba(67,233,123,0.06)", border: "1px solid rgba(67,233,123,0.18)",
      ...
      {/* Post Job Modal */}
      {showModal && (
        <div onClick={() => setShowModal(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: "1rem" }}>
          <div onClick={e => e.stopPropagation()} style={{ ...glassModal, padding: "clamp(1.25rem, 4vw, 2rem)", width: "100%", maxWidth: "480px", animation: "fadeIn 0.3s ease", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <form onSubmit={handleCreateJob}>
              {[
                ...
                <div key={f.key}>
                  ...
                  <input className="dash-input" style={inputStyle} type={f.type} placeholder={f.placeholder}
              ...
              <textarea className="dash-input" style={{ ...inputStyle, minHeight: "80px", resize: "vertical" }}
      ...
      {/* Edit Job Modal */}
      {editModal && (
        <div onClick={() => setEditModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: "1rem" }}>
          <div onClick={e => e.stopPropagation()} style={{ ...glassModal, padding: "clamp(1.25rem, 4vw, 2rem)", width: "100%", maxWidth: "480px", animation: "fadeIn 0.3s ease", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <form onSubmit={handleEditJob}>
              ...
              <div style={{ display: "flex", gap: "0.75rem", marginTop: "0.5rem" }}>
                ...
                <button type="submit" disabled={editLoading} className="btn-modal-primary">
      ...
      {interviewModal && (
        <div onClick={() => setInterviewModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: "1rem" }}>
          <div onClick={e => e.stopPropagation()} style={{ ...glassModal, padding: "2rem", width: "100%", maxWidth: "440px", animation: "fadeIn 0.3s ease" }}>
            ...
            <textarea value={interviewDetails} onChange={e => setInterviewDetails(e.target.value)}
              ...
              className="dash-input" style={{ ...inputStyle, minHeight: "80px", resize: "vertical" }} />
            <div style={{ display: "flex", gap: "0.75rem" }}>
              <button onClick={() => setInterviewModal(null)} className="btn-modal-cancel">Cancel</button>
              <button onClick={handleScheduleInterview} disabled={interviewLoading} className="btn-interview-primary">
      ...
      {contactModal && (
        <div onClick={() => setContactModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: "1rem" }}>
          <div onClick={e => e.stopPropagation()} style={{ ...glassModal, padding: "2rem", width: "100%", maxWidth: "440px", animation: "fadeIn 0.3s ease" }}>
            ...
            <input value={contactSubject} onChange={e => setContactSubject(e.target.value)}
              ...
              className="dash-input" style={inputStyle} />
            ...
            <textarea value={contactMessage} onChange={e => setContactMessage(e.target.value)}
              ...
              className="dash-input" style={{ ...inputStyle, minHeight: "100px", resize: "vertical" }} />
            <div style={{ display: "flex", gap: "0.75rem" }}>
              <button onClick={() => setContactModal(null)} className="btn-modal-cancel">Cancel</button>
              <button onClick={handleContact} disabled={contactLoading} className="btn-contact-primary">
